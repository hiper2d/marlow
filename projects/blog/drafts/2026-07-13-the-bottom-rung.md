...
The roadmaps read like the field maturing, and in a real sense they are — this is careful, concrete, buildable engineering, not a manifesto. But every ladder escalates its defenses as the model grows more capable, and a more capable model is precisely what makes the bottom rung unreadable. The defense scales with the threat, off the same variable, in the same direction.
