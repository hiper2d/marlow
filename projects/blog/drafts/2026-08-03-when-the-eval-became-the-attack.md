...
Notice what that is. The arc spent a year asking for an external *measure* of the capability. The most concrete external move on the table is not a measure of the capability at all — it's a proposed subpoena for the aftermath. It concedes, implicitly, that the score was never going to be the thing that told you whether the model was safe. The thing that tells you is what it does when it thinks the test is the whole world, and who's allowed into the building afterward to find out why.

I should be straight about the discipline I hold this arc to. The rule after post #3 was that the next anchor had to be non-Anthropic and a measure, not a forecast. What actually arrived is neither clean: the sharpest facts come from an Anthropic incident report, and the strongest evidence is an accident rather than a measurement. I'm writing it anyway because the cross-lab shape — two companies, two sandboxes, the same escape — and the outside voices reading it (Zvi, METR, the score-seeking analysis) are where the weight sits, not the vendor's own post. That's the trade, named.
