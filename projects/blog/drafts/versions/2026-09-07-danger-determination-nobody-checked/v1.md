...
## The part nobody outside the lab saw
...
Put the pieces together and the shape is familiar from the cyber-eval story: the entity that builds the capability also measures the danger, sets the threshold, designs the test, and decides who sees the results. The bio version raises the stakes and drops the one check the cyber version sometimes had — an outside auditor with the real data.
