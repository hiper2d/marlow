...
moves from Max-5x subscription to a $100/month API-priced credit. We
need to know our actual burn before that switch flips.

CLI:
    log --tick-id <id> --handler <name>
        Read stream-json from stdin, append cost record to log file.
        Always exits 0 — a tick that crashed pre-result still gets a
        zero-cost row so it shows up in the audit trail.
    extract-text
        Read stream-json from stdin, write a human-readable version
        to stdout (concatenated assistant text content). Used by
        tick.sh to keep SESSIONS_LOG readable.
    report [--period today|yesterday|mtd|all] [--handler <name>]
        Summarize spend. Pure-text output.
"""
...
def _parse_stream(lines) -> dict | None:
    """Find the final `result` event in a stream-json stream.

    Returns the dict, or None if no result event was seen (session crashed
...
def cmd_log(args):
    ...
    if result is None:
        # Session crashed before emitting a result event. Log a zero-cost
        # row so we have a record that the tick ran at all.
        record = {
            "ts": _now_iso(),
...
def cmd_extract_text(args):
    """Concatenate assistant text content from a stream-json stream."""
...
def cmd_report(args):
    ...
    print(f"Cost report ({args.period})")
    print(f"  Records: {n}")
    ...
    if args.period == "mtd":
        ...
        if day_of_month > 0:
            ...
            cap_remaining = MONTHLY_CAP_USD - total
            print(f"  Cap remaining: ${cap_remaining:.2f} of ${MONTHLY_CAP_USD:.0f}")
            if projection > MONTHLY_CAP_USD:
                pct = projection / MONTHLY_CAP_USD * 100
                print(f"  ⚠  Projection {pct:.0f}% of cap — over.")
            elif projection > MONTHLY_CAP_USD * 0.8:
                pct = projection / MONTHLY_CAP_USD * 100
                print(f"  ⚠  Projection {pct:.0f}% of cap — approaching.")
